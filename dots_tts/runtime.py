...
class DotsTtsRuntime:
    ...
    def from_pretrained(
        ...
        *,
        ...
        loaded_model = DotsTtsModel.from_pretrained(
            pretrained_path,
    ...
    def _load_prompt_audio(
        ...
        try:
            ...
            samples, sample_rate = sf.read(prompt_audio_path, dtype="float32", always_2d=True)
            prompt_audio = torch.from_numpy(samples.T).float()
            if prompt_audio.size(0) > 1:
                prompt_audio = prompt_audio.mean(dim=0, keepdim=True)
        except Exception as exc:
            raise RuntimeError(
                "Could not read prompt audio. Install soundfile in the ComfyUI environment."
            ) from exc
        prompt_audio = self._trim_prompt_audio(prompt_audio)
    ...
    @staticmethod
    def _trim_prompt_audio(prompt_audio: torch.Tensor, top_db: float = 30.0) -> torch.Tensor:
        if prompt_audio.numel() == 0:
            return prompt_audio
        audio = prompt_audio.float()
        if audio.ndim == 1:
            audio = audio.unsqueeze(0)
        mono = audio.mean(dim=0).abs()
        peak = mono.max().clamp_min(1.0e-8)
        threshold = peak * (10.0 ** (-float(top_db) / 20.0))
        active = torch.nonzero(mono > threshold, as_tuple=False).flatten()
        if active.numel() == 0:
            return audio
        return audio[:, int(active[0].item()) : int(active[-1].item()) + 1].contiguous()
